...
def credential_Issuing(params, pub, ciphertext, issuer_params):
	...
	w[1] = o.random() # beta
	w[2] = o.random() # x1b
	w[3] = o.random() # r_prime
	w[4] = o.random() # x0
	w[5] = o.random() # x0_bar

	c = to_challenge([g, h, pub, a, b, X1, X1b, new_a, new_b, Cx0, 
		w[0] * h, w[1] * X1, w[2] * h, w[1] * g, w[3] * g + w[2] * a,
	    w[3] * pub + w[2] * b + w[4] * u, w[4] * g + w[5] * h])

	rs = range(6)
	rs[0] = (w[0] - c * x1) % o
	rs[1] = (w[1] - c * beta) % o
	rs[2] = (w[2] - c * x1b) % o
	rs[3] = (w[3] - c * r_prime) % o
	rs[4] = (w[4] - c * x0) % o
	rs[5] = (w[5] - c * x0_bar) % o
...
def credential_show(params, issuer_pub_params, u, u_prime, v):
    ...
    wv = o.random()
    V = r * (-g) + z1 * X1
    c = to_challenge([g, h, Cv, Cup, wv*u + wz1*h, wr*(-g) + wz1*X1])
    rr = (wr - c * r) % o
    rz1 = (wz1 - c * z1) % o
    rv = (wv - c * v) % o
...
def credential_show_pseudonym(params, issuer_pub_params, u, u_prime, v, service_name):
    ...
    wz1 = o.random()
    wv = o.random()
    wz2 = o.random() # service
    V = r * (-g) + z1 * X1
    c = to_challenge([g, h, Cv, Cup, Cp, wv*u + wz1*h, wr*(-g) + wz1*X1, wv*N + wz2*h])
    rr = (wr - c * r) % o
    rz1 = (wz1 - c * z1) % o
    rv = (wv - c * v) % o
    rz2 = (wz2 - c * z2) % o # service

    proof = (c, rr, rz1, rv, rz2)
...
def credential_show_verify_pseudonym(params, issuer_params, pseudonym, tag, proof, service_name):
    ...
    (c, rr, rz1, rv, rz2) = proof
    (u, Cv, Cup, Cp) = tag

    V = x0*u + x1*Cv - Cup
    c_prime = to_challenge([g, h, Cv, Cup, Cp, c*Cv + rv*u + rz1*h, c*V + rr*(-g) + rz1*X1, c*Cp + rv*N + rz2*h])
